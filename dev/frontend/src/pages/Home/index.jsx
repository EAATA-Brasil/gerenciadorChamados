...
function Home() {
  ...
  const handleCloseModal = () => {
    ...
    setNewComment({ autor: "", conteudo: "" });
    removeCommentFile();
    setContextMenuState({ visible: false, x: 0, y: 0, src: "" });
  };
  ...
  const handleDownloadImage = () => {
    if (!contextMenuState.src) return;

    const link = document.createElement("a");
    link.href = contextMenuState.src;
    link.download = contextMenuState.src.split("/").pop() || "image";
  ...
  const handleOpenImageInNewTab = () => {
    if (contextMenuState.src) {
      window.open(contextMenuState.src, "_blank", "noopener,noreferrer");
    }
    setContextMenuState((prev) => ({ ...prev, visible: false }));
  };
