...
function getBackendExecutable() {
  ...
  if (app.isPackaged) {
    ...
    // Em desenvolvimento, o backend é executado via npm, não o executável
    return path.join(__dirname, "..", "backend"); // Retorna o diretório do backend
  }
}
...
function startBackend() {
  ...
  if (app.isPackaged) {
    ...
    // Em desenvolvimento, execute 'npm run start:dev' no diretório do backend
    spawnCmd = process.platform === "win32" ? "npm.cmd" : "npm";
    spawnArgs = ["run", "start:dev"];
    cwd = backendPath; // O diretório do backend é o CWD
  }
